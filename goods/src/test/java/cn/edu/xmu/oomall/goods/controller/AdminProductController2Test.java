...
//                .andDo(MockMvcResultHandlers.print());
    ...
    public void getOnsaleTest1() throws Exception{
        ...
        retObj.setData(Shop.builder().id(10L).name("商铺10").type((byte)0).build());
        Mockito.when(shopDao.getShopById(10L)).thenReturn(retObj);
        this.mockMvc.perform(MockMvcRequestBuilders.get("/shops/1/onsales/1")
                        .header("authorization", adminToken)
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        )
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void getOnsaleTest2() throws Exception{
        ...
        retObj.setErrno(0);
        retObj.setErrmsg("成功");
        retObj.setData(Shop.builder().id(4L).name("商铺10").type((byte)0).build());
        Mockito.when(shopDao.getShopById(4L)).thenReturn(retObj);
        this.mockMvc.perform(MockMvcRequestBuilders.get("/shops/4/onsales/3")
                        .header("authorization", adminToken)
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                )
                .andExpect(MockMvcResultMatchers.status().isOk())
